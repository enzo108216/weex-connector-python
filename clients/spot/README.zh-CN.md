...
这是一个面向 WEEX Spot API 的客户端库，帮助开发者通过以下三个独立入口以编程方式接入 WEEX 现货交易：
...
- REST API 端点：
  - `/api/*`
  - 服务分组：`GeneralApi`、`MarketApi`、`TradeApi`、`AccountApi`、`RebateApi`
- WebSocket 端点：支持鉴权请求-响应流程以及公开市场数据流。
- 提供可直接运行的示例和连接器内本地测试，便于快速上手。
- 范围说明：本 SDK 有意不包含 `POST /api/v3/order/batch`。
...
这些包目前通过源码方式安装。请先在仓库根目录安装共享运行时，再安装 Spot 连接器：
...
如果你需要 Spot 专属的开发依赖，请在 `./common` 可用后，以 `dev` extra 安装该包。
...
对于 rebate 或 partner 流程，你也可以提供一套独立凭证，并通过 `env_prefix="WEEX_PARTNER_"` 创建第二个客户端：
...
- [REST API 源码](./src/weex_spot_sdk/rest_api/rest_api.py)
- [Websocket API 源码](./src/weex_spot_sdk/websocket_api/websocket_api.py)
- [Websocket Streams 源码](./src/weex_spot_sdk/websocket_streams/websocket_streams.py)
...
所有 REST API 端点都通过 [`rest_api`](./src/weex_spot_sdk/rest_api/rest_api.py) 模块提供。REST API 可用于获取市场数据、管理交易以及访问账户信息。部分端点需要使用你的 WEEX 凭证进行鉴权。
...
- `base_path`：REST 基础 URL。默认值为 `https://api-spot.weex.com`。
...
当你需要接入不同的 WEEX 环境时，可使用 `base_path` 覆盖默认的 Spot REST 端点。
...
- Spot 签名覆盖位于 [`./tests`](./tests/)。
- 示例程序位于 [`./examples`](./examples/)。
- 如果你为 Spot 包安装了测试依赖，请在 `./tests` 中运行本地测试。

<a id="migration-guide"></a>

## 迁移指南

如果你正从旧的独立版 `weex-spot-sdk` 布局升级：

- 对外公开的导入路径仍然是 `from weex_spot_sdk import Spot`。
- 共享运行时代码现在位于 `../../common`，因此在安装 Spot 包前必须先安装 `weex-common`。
- 这类范围说明，例如被排除的 batch order 端点，会继续记录在本 README 中。
...
2. 将 Spot 专属行为保留在 [`./src/weex_spot_sdk`](./src/weex_spot_sdk)。
